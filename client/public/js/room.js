...
cursorColorSelect.addEventListener("change", handleCursorColorChange);
const cursorEraserButton = document.getElementById("cursorEraser");
const cursorEraserSizeInput = document.getElementById("cursorEraserSize");
const output = document.getElementById("output");
cursorEraserSizeInput.style.visibility = "hidden";
...
function handleCursorColorChange(e) {
    const colors = e.target.value;
    color = colors;
    drawsize = 3;
}

cursorEraserButton.addEventListener("click", () => {
    color = "white";
    cursorEraserSizeInput.addEventListener("change", (e) => {
        const size = e.target.value;
        output.innerHTML = size;
        console.log(size);
        drawsize = size;
    });
    cursorEraserSizeInput.style.visibility =
        cursorEraserSizeInput.style.visibility === "hidden" ? "visible" : "hidden";
...
whiteboardButt.addEventListener("click", () => {
    if (boardVisisble) {
        ...
        cursorEraserSizeInput.style.visibility =
            cursorEraserSizeInput.style.visibility = "hidden";
        output.style.visibility = output.style.visibility = "hidden";
        navigator.mediaDevices
            ...
            .then((localstream) => {
                ...
                videoAllowed = 1;
                videoButt.style.backgroundColor = "#0495f7";
        ...
        await conc.forEach(sid => {
            ...
            connections[sid].ontrack = function (event) {
                ...
                if (!document.getElementById(sid)) {
                    ...
                    videoOff.innerHTML = `<i class="fa-solid fa-user" style="color: gray; font-size: 150px;"></i>`;
...
socket.on("message", (msg, sendername, time) => {
    ...
    if (sender === username.toUpperCase()) {
        chatRoom.innerHTML += `<div class="message_author">
          <div class="info">
            <div class="username">${sender}<span style="font-size:10px;padding-left:5px;">(You)</span></div>
...
videoButt.addEventListener("click", () => {
    if (videoAllowed) {
        ...
        navigator.mediaDevices
            ...
            .catch((error) => {
                console.error("Error acquiring camera access:", error);
            });
    }
});
...
audioButt.addEventListener("click", () => {
    if (audioAllowed) {
        ...
        audioAllowed = 1;
        audioButt.style.backgroundColor = "#0495f7";
        if (mystream) {
            mystream.getTracks().forEach((track) => {
...
const show = document.getElementById("right-cont");
